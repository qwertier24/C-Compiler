...
def getaddr(var, table, output, des=None):
    ...
    if type(var) == int or type(var) == float:
        ...
        if "[" in var:
            [name, pos] = var.split('[')
            if "(" in pos or not pos[:-1].isdigit():
                pos = pos[:-1]
                output.write("  movl %d(%%ebp), %%eax\n"%(-table[pos].addr))
                if name in table:
                    output.write("  lea %s, %%eax\n"%(str(-table[name].addr) + "(%ebp, %eax, 4)"))
                    return "%eax"
                elif name in table[".."]:
                    output.write("  lea %s, %%eax\n"%(str(table[".."][var].addr+4*pos) + "(%eds)"))
                    return "%eax"
            else:
                pos = int(pos[:-1])
                if name in table:
                    output.write("  lea %s, %%eax\n"%(str(-table[name].addr+4*pos) + "(%ebp)"))
                    return "%eax"
                elif name in table[".."]:
                    output.write("  lea %s, %%eax\n"%(str(table[".."][name].addr+4*pos) + "(%eds)"))
                    return "%eax"
        if var in table:
            output.write("  lea %s, %%eax\n"%(str(-getVar(var, table).addr) + "(%ebp)"))
            return "%eax"
        if var in table[".."]:
            return "$"+var
...
def outputAssembler(codes, table, output):
    ...
    for code3 in codes:
        ...
        if code3.op == "function":
            ...
            output.write("  call %s\n"%(code3.arg1))
            output.write("  addl $%d, %%esp\n"%(4*code3.arg2))


def compile(codes):
    open("temp.c", "w").write(codes)
    scanner = Scanner(open("temp.c", "rb"))
    ...
    asm = open("temp.s", "w")
    outputAssembler(codes, table, asm)
    asm.close()
