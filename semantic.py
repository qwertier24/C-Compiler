...
class Symbol:
    def __init__(self, mold, addr, width, dim=[], init=None):
        self.mold = mold
        self.addr = addr + width - 4
        self.width = width
        self.dim = dim[:]
        self.init = init
...
class Semantic:
    def __init__(self, cfl_file_name):
        if type(cfl_file_name) != ContextFreeLanguage:
            self.cfl = pickle.load(open(cfl_file_name, "rb"))
        else:
            self.cfl = cfl_file_name
    ...
    def enterString(self, s):
        table = self.tableStack[0]
        var = "_s" + str(self.stringCount)
        table[var] = Symbol("str", self.offsetStack[0], len(s), [], s)
        self.offsetStack[0] += len(s)
        self.offsetStack[0] = (self.offsetStack[0]+3)//4*4
        self.stringCount += 1;
        return var
    def addTable(self):
    ...
    def f91(self, rootInfo, childInfo):
        ...
        rootInfo.mold = "float"
        rootInfo.code += childInfo[0].code
    def f93(self, rootInfo, childInfo):
        # [const] --> "STR" {f93()}
        rootInfo.name = self.newTemp("int", 4)
        rootInfo.code.append(TriAddr("=", "$"+self.enterString(childInfo[0].name), None, rootInfo.name))
    ...
    def f103(self, rootInfo, childInfo):
        # [factor] --> "&" "IDN" {f103()}
        rootInfo.name = self.newTemp("int", 4)
        rootInfo.code += childInfo[1].code
        rootInfo.code.append(TriAddr("=", "$"+childInfo[1].name, None, rootInfo.name))
    def f104(self, rootInfo, childInfo):
        # [factor] --> "&" [arr_find] {f104()}
        rootInfo.name = self.newTemp("int", 4)
        rootInfo.code += childInfo[1].code
        rootInfo.code.append(TriAddr("=", "$"+childInfo[1].addr+"["+str(childInfo[1].name)+"]", None, rootInfo.name))
